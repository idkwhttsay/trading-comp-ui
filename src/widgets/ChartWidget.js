...
const ChartWidget = ({ selectedStock }) => {
  ...
  const minPrice =
    candlestickData.length > 0 ? Math.min(...candlestickData.map((d) => d.y[2])) * 0.98 : undefined;
  const maxPrice =
    candlestickData.length > 0 ? Math.max(...candlestickData.map((d) => d.y[1])) * 1.02 : undefined;
