...
export default class DataHelper {
    ...
    static getStockOrders(ticker) {
        return SampleOrderBookData.find((item) => item.Ticker === ticker);
    }
    ...
    /**
     * Fetches and returns all open orders for a given team
     * @param {string} APIKey - The API key for the team
     * @returns {Object[]} - An array of open order objects
     */
    static getOpenOrders(APIKey) {
        if (APIKey === '12345') {
            // Sample API response for open orders
            return [
                { ticker: 'AAPL', is_buy: true, price: 145.25, quantity: 10 },
                { ticker: 'GOOGL', is_buy: false, price: 2723.5, quantity: 5 },
                { ticker: 'TSLA', is_buy: true, price: 650.75, quantity: 8 },
